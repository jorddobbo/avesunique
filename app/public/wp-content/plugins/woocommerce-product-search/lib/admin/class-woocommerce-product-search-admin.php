...
class WooCommerce_Product_Search_Admin {
	...
	public static function wp_init() {
		...
		if (
			...
			// Create a FULLTEXT index on wp_postmeta.meta_value
			//$results = $wpdb->get_results( "SHOW INDEX FROM $wpdb->postmeta WHERE Key_name = 'wps_ft_meta'" );
			//if ( empty( $results ) ) {
			//	$query = "ALTER TABLE $wpdb->postmeta ADD FULLTEXT INDEX wps_ft_meta (meta_value)";
			//	if ( $wpdb->query( $query ) === false ) {
			//		error_log( $wpdb->last_error );
			//		$wps_ft_db_update_errors[] = sprintf( __( "The table <code>$wpdb->postmeta</code> could not be updated; <code>%s</code> while running the query <code>%s</code>", WOO_PS_PLUGIN_DOMAIN ), $wpdb->last_error, $query );
			//	} else {
			//		$wps_ft_db_update_notices[] = sprintf( __( "The table <code>$wpdb->postmeta</code> has been updated using the query <code>%s</code>", WOO_PS_PLUGIN_DOMAIN ), $query );
			//	}
			//}
	...
	public static function woocommerce_product_search() {
		...
		if ( !isset( $options[WooCommerce_Product_Search::FT_MIN_WORD_LEN] ) || ( $options[WooCommerce_Product_Search::FT_MIN_WORD_LEN] !== $ft_min_word_len ) ) {
			...
			update_option( 'woocommerce-product-search', $options );
			// this is set here and automatically updated so the variable doesn't need to be obtained through the method on each query
		}
