...
class WooCommerce_Product_Search_Widget extends WP_Widget {
	...
	function form( $instance ) {
		...
		echo '<p>';
		echo sprintf( '<label title="%s">', sprintf( __( 'Search results should include matches in SKU.', WOO_PS_PLUGIN_DOMAIN ) ) );
		printf(
			'<input type="checkbox" id="%s" name="%s" %s />',
			$this->get_field_id( 'sku' ),
			$this->get_field_name( 'sku' ),
			$sku == 'yes' ? ' checked="checked" ' : ''
		);
		echo ' ';
		echo __( 'Search in SKU', WOO_PS_PLUGIN_DOMAIN );
		echo '</label>';
		echo '</p>';
